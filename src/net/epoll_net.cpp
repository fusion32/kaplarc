...
struct Socket{
	...
	struct AsyncOp		ops[SOCKET_MAX_OPS];
	struct AsyncOp		*rd_queue;
	struct AsyncOp		*wr_queue;
	struct mutex		*lock;
};
...
static Socket *socket_handle(int fd){
	struct socket *sock;
...
uint32 socket_remote_address(Socket *sock)
{
	...
	return ((sockaddr_in*)&sock->addr)->sin_addr.s_addr;
}
...
bool socket_async_read(Socket *sock, char *buf, int len, Callback cb, void *udata){
	struct async_op *op, **it;
...
bool socket_async_write(Socket *sock, char *buf, int len, Callback cb, void *udata){
	struct async_op *op, **it;
